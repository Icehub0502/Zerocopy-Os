...
public class Client {
    ...
    private final String foldercopy = "D:/os-main/Client/copy/"; // <<<<<<<<<<<<<<<<<<<<<<<<
    private final String folderzero = "D:/os-main/Client/zero/"; // <<<<<<<<<<<<<<<<<<<<<<<<
    String[] filename = new String[10000];

    public final void connection() {
        try {
            socket = new Socket("192.168.1.45", 5000); // <<<<<<<<<<<<<<<<<<<<<<<<
            dis = new DataInputStream(socket.getInputStream());
            dos = new DataOutputStream(socket.getOutputStream());
            socketChannel = SocketChannel.open(new InetSocketAddress("192.168.1.45", 5001)); // <<<<<<<<<<<<<<<<<<<<<<<<
        } catch (IOException e) {
        }
    }
    ...
    public final void zeroCopy(String filePath, long size) {
        ...
        try {
            ...
            while (currentRead < size
                    && (read = destination.transferFrom(socketChannel, currentRead, size - currentRead)) != -1)
                currentRead += read;
